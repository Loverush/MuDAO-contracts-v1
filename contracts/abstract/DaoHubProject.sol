...
/** 
  @notice A contract that inherits from DaoHubProject can use DaoHub as a business-model-as-a-service.
...
*/
abstract contract DaoHubProject is IERC721Receiver, Ownable {
    ...
    */
	function pay(
		...
		require(projectId != 0, 'DaoHubProject::pay: PROJECT_NOT_FOUND');

		// Get the terminal for this contract's project.
		ITerminal _terminal = terminalDirectory.terminalOf(projectId);

		// There must be a terminal.
		require(_terminal != ITerminal(address(0)), 'DaoHubProject::pay: TERMINAL_NOT_FOUND');
    ...
    */
	function _takeFee(
		...
		require(projectId != 0, 'DaoHubProject::takeFee: PROJECT_NOT_FOUND');
		// Find the terminal for this contract's project.
		ITerminal _terminal = terminalDirectory.terminalOf(projectId);

		// There must be a terminal.
		require(_terminal != ITerminal(address(0)), 'DaoHubProject::takeFee: TERMINAL_NOT_FOUND');

		// There must be enough funds in the contract to take the fee.
		require(address(this).balance >= _amount, 'DaoHubProject::takeFee: INSUFFICIENT_FUNDS');
